...
class CreateRecommendationForAnotherUser(BaseAction):
    ...
    def send_recommendation_to_recommendee(cls, recommender, recommendee, recommendation):
        if cls.is_trusted:
            ...
            SmsSender.send_to_user(
                recommendee.phone,
                f"{recommender.username} recommended '{recommendation.name}' to you. Text back 'r{recommendation.id}' "
                f"if you would like to add this recommendation and them as a trusted user."
            )
